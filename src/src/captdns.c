...
static void  captdnsRecv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) {
    ...
    for (i=0; i< lwip_ntohs(hdr->qdcount); i++) 
    {
        ...
        printf("DNS: Q (type 0x%X class 0x%X) for %s\n", lwip_ntohs(qf->type), lwip_ntohs(qf->class), buff);

        if (lwip_ntohs(qf->type)==QTYPE_A) {
            ...
            rf->class = lwip_htons(QCLASS_IN);
            rf->ttl = 0;
            rf->rdlength = lwip_htons(4); //IPv4 addr is 4 bytes;
            ...
            *rend++=ip4_addr1(&ip_info.ip);
            *rend++=ip4_addr2(&ip_info.ip);
            *rend++=ip4_addr3(&ip_info.ip);
            *rend++=ip4_addr4(&ip_info.ip);
            ...
            *rend++=0;
            ...
            rf->type = lwip_htons(QTYPE_URI);
            rf->class = lwip_htons(QCLASS_URI);
            rf->ttl = lwip_htons(0);
            rf->rdlength = lwip_htons(4+16);
            uh->prio = lwip_htons(10);
            uh->weight = lwip_htons(1);
